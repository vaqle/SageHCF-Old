...
class LootBoxCommand extends PluginCommand
{
	...
	public function __construct($name, Sage $plugin)
	{
		...
		$this->setDescription("§rA command used to distribute specific lootboxes");
	}
	...
	public function execute(CommandSender $sender, string $label, array $args)
	{
		...
		if ($sender instanceof SagePlayer) {
			if ($sender->isOp() || $sender->hasPermission("lootbox.cmd") || $sender instanceof ConsoleCommandSender) {
				if (count($args) < 3) {
					$sender->sendMessage("§l§c[!] §r§cNo such lootbox called “null” was found \n§7if you believe this is an error report this to vaqle");
					$sender->sendMessage("§r§e/lootbox <player> <type> <amount>");
					$sender->sendMessage("§r§6LOOTBOX IDS: §r§gSage§7, §dSummer");
				} else {
					if (isset($args[0]) && ($player = Server::getInstance()->getPlayer($args[0])) && is_string($args[1]) && isset($args[2]) && is_numeric($args[2])) {
						if ($player instanceof SagePlayer) {
							PItemManager::giveLootBox($player, (string)$args[1], (int)$args[2]);
							$sender->sendMessage("§e§l(!) §r§eYou have successfully given §6x" . $args[2] . " " . $args[1] . " Lootboxe(s) §eto §6" . $player->getName());
						}
					} else {
